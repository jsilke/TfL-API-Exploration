...
def store_response_json(
    json_object: dict, file_name: str = MOST_RECENT, directory: str = DIRECTORY
) -> None:
    """
...
def get_valid_lines(
    url: str = ALL_MODES_URL, transportation_modes: str = "bus, tube"
) -> re.Response:
    """
...
def get_line_stations(
    id: str = "victoria", tfl_only: bool = False, url: str = BASE_LINE_URL
):
    _FULL_URL = f"{url}{id}/StopPoints"
    return get_json(
        _FULL_URL, parameters={"tflOperatedNationalRailStationsOnly": tfl_only}
    )
...
def get_stop_points(
    origin: str = "Heathrow Airport",
    destination: str = "Tower Bridge",
    url: str = STOP_POINTS_URL,
) -> tuple[str, str]:
    """Get stop point coordinates for origin and destination and return them as a tuple."""
    origin, destination = get_json(url, parameters={"query": origin}), get_json(
        url, parameters={"query": destination}
    )

    # Get the latitude and longitude as comma separated strings for origin and destination.
    origin_lat_lon, destination_lat_lon = (
        f"{origin['matches'][0]['lat'], origin['matches'][0]['lon']}",
...
def plan_journey(
    from_and_to: tuple[str, str], mode: str, url: str = JOURNEY_RESULTS_URL
):
...
def get_bikepoint_info(url: dict = ALL_BIKEPOINTS_URL):
    ...
    for _bike_point in _bike_points:
        _empty_dock_sum += int(_bike_point["additionalProperties"][-4]["value"])
